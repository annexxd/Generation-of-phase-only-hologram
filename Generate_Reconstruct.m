function [RECON_image, R_HOLO, I_HOLO] = Generate_Reconstruct(IMG,select,t)
%when carry out B,select=0,carry out C, select=1,carry out D, select=2
%t means down sampling parameter
...
%输入两个参数时，t为默认值6；输入三个参数时，t为用户定义的值
if nargin==2
...
H=Holo_gen(I,z1,psize,lamda);  %Generate hologram（Contain processing phase-only Hologram）
R_HOLO=real(H);
I_HOLO=imag(H);
R_HOLO=mat2gray(R_HOLO(sy:ey,sx:ex));%仅展示中间512*512的部分
I_HOLO=mat2gray(I_HOLO(sy:ey,sx:ex));%这是需要展示的实部和虚部图

%Reconstruct hologram to image, extract magnitude component
RECON_image=abs(Holo_recon(H,z1,psize,lamda));
